...
#two ways of merging the EVI and soil datasets
#first, join based on multiple column on the EVI_data and soil_data_long_outliers_replaced
new1 <- left_join(soil_data_long_outliers_replaced, EVI_data, by=c('Rs'='Rs', 'Crop'='Crop'))
new1

#second, create unique identifier for both datasets and join based on the unique identifier
  #create unique identifier for the soil_data_long_outliers_replaced
soil_data_with_id <- soil_data_long_outliers_replaced %>%                                        
                    group_by(Crop, Rs) %>%
                    mutate(ID = cur_group_id())
soil_data_with_id

  #create unique identifier for the EVI_data
EVI_data_with_id <- EVI_data %>%                                        
                  group_by(Crop, Rs) %>%
                  mutate(ID = cur_group_id())
EVI_data_with_id

  #join both dataset based on the unique identifier ID
new2 <- left_join(soil_data_with_id, EVI_data_with_id, by=c('ID'='ID'))
new2
